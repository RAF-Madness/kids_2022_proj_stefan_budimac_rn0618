...
public class QuitCommand implements CLICommand {
    ...
    public void execute(String args) {
        ...
        for (Map.Entry<Integer, NodeInfo> entry : AppConfig.state.getNodes().entrySet()) {
            ...
            QuitMessage quitMessage = new QuitMessage(AppConfig.info.getNodeInfo(), entry.getValue());
            MessageUtil.notifyQuit(quitMessage, barrier);
        }
        LeaveMessage leaveMessage = new LeaveMessage(AppConfig.info.getNodeInfo(), new NodeInfo(AppConfig.BOOTSTRAP.getPort(), AppConfig.BOOTSTRAP.getIpAddress(), -1, "", ""));
